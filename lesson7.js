...
function startGame() {
    ...
    setTimeout(appearingWalls, 9000);
}
...
function move() {
    ...
    // new 3) На следующем ходу нет препятствия
    //console.log(new_unit);
    if (!isSnakeUnit(new_unit) && new_unit !== undefined && !new_unit.classList.contains('wall-unit')) {
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        score++;
        document.querySelector('.count span').innerText = score
    }
    return check;
}
...
//периодическое появление препятствий
function appearingWalls() {
    var wallInterval = Math.floor(Math.random() * 5000 + 2000);
    setInterval(createWall, wallInterval);
}

//появление одного препятствия
function createWall() {
